...
# using when have skip connection 
...
class BaseMLP(ABC):
    ...
    def print_fmt(self,Value):
        if Value is None or len(Value) ==0:
            return float('nan')
        return Value[-1]
    def fit(self, X = None, y = None, X_val = None, y_val = None, dataset = None, val_dataset = None, lr = 0.01, n_epochs = 100, batch_size = 1, verbose = 0, validation_split = None, is_shuffle = True, optimizer = 'SGD', criterion = 'MSE'):
        ...
        if self.val_dataset is not None:
            self.Val_Loader = DataLoader(self.val_dataset,batch_size=len(self.val_dataset))
